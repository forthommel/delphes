...
DelphesModel::DelphesModel(std::string_view model_name) : fName(model_name) {}
...
void DelphesModel::throwAttachingFailure(std::string_view field_name, std::string_view details) const noexcept(false)
{
  ...
  for(const auto &[field_name, _] : fMemorySegments)
    os << "\n - '" << field_name << "' of type '" << fFieldTypes.at(field_name) << "'";
  if(!details.empty())
    os << "\nDetails: " << details;
  throw std::runtime_error(os.str());
}

DelphesModel::FieldName::FieldName(std::string_view field_label) : fLabel(field_label)
{
  // one-liner with C++23
