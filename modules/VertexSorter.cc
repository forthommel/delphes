...
void VertexSorter::Process()
{
  ...
  {
    const Candidate &cluster = fInputArray->at(iCluster);
  ...
  if(fMethod == "BTV")
  {
    ...
    for(const Candidate &candidate : *fTrackInputArray)
    {
      ...
      for(const Candidate &jetCandidate : *fJetInputArray)
      {
    ...
    std::sort(sortedClusterIDs.begin(), sortedClusterIDs.end(), secondDescending);
  }
  else if(fMethod == "GenClosest")
  {
    ...
    std::sort(sortedClusterIDs.begin(), sortedClusterIDs.end(), secondAscending);
  }
  ...
  {
    Candidate &cluster = fOutputArray->emplace_back(fInputArray->at(clusterIDToIndex.at(itSortedClusterIDs->first)));
    if(fMethod == "BTV")
